# Diário de Desenvolvimento

---

## Data: 13/12/2025 – 15:00 - 17:00

**Integrantes que trabalharam:**
- Adria Tavares
- Ewerton Evaristo
- Elioenai Adalberto

**Resumo do que foi feito:**
- Planejamos como seria feito e dividido o trabalho, e criamos as contas que seriam usadas no GitHub.
- Divisão de trabalho:
  - **Adria**: Manipulação dos menus
  - **Ewerton**: Planejamento da divisão e criação da interface dos menus
  - **Elioenai**: Criação e planejamento das classes dos produtos

---

## Data: 15/12/2025 – 18:30 - 20:00

**Integrantes que trabalharam:**
- Elioenai Adalberto

**Resumo do que foi feito:**
- Criação do arquivo das classes
- Hash do commit: 00c5bf2
---

## Data: 16/12/2025 – 22:00 - 23:21

**Integrantes que trabalharam:**
- Ewerton Evaristo

**Resumo do que foi feito**
- Criação do arquivo que manipula o objeto principal e cria as instâncias das classes.

* Hash do commit: 1751951

---
## Data: 17/12/2025 – 20:00 - 21:21

**Integrantes que trabalharam:**
- Adria Tavares

**Resumo do que foi feito**
- Criação dos arquivos responsáveis pelos menus

  * Hash do commit: 1eb987a
